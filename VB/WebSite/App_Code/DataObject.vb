...
Public Class DataObject
    ...
    Private ReadOnly origQuery As IQueryable(Of Contact)

    Private query As IQueryable(Of Contact)
    Private keyExpression As String

    Private Const pageSize As Int32 = 23 ' approximate page size
    ...
    Private count_Renamed As Int32 = -1 ' row count
    Private keys As Array ' array of keys
    Private rows As Array ' array of records
    Private startKeyIndex As Int32 = -1 ' relative key index
    Private startRowIndex As Int32 = -1 ' relative row index
    ...
    Private Sub IListServer_Apply(ByVal filterCriteria As DevExpress.Data.Filtering.CriteriaOperator, ByVal sortInfo As ICollection(Of ServerModeOrderDescriptor()), ByVal groupCount As Integer, ByVal groupSummaryInfo As ICollection(Of ServerModeSummaryDescriptor), ByVal totalSummaryInfo As ICollection(Of ServerModeSummaryDescriptor)) Implements IListServer.Apply
        ...
        If sortInfo IsNot Nothing Then
            ...
            For Each info As ServerModeOrderDescriptor() In sortInfo
                ...
                End If
                Dim propertyName As String = (TryCast(info(0).SortExpression, OperandProperty)).PropertyName

                If first Then
                    If info(0).IsDesc Then
                        oq = LinqHelper.OrderByDescending(Me.query, propertyName)
                    Else
                        oq = LinqHelper.OrderBy(Me.query, propertyName)
                    End If

                    first = False
                Else
                    If info(0).IsDesc Then
                        oq = LinqHelper.ThenByDescending(oq, propertyName)
                    Else
                        oq = LinqHelper.ThenBy(oq, propertyName)
                    End If
                End If
            Next info

            Me.query = oq.AsQueryable(Of Contact)()
        End If
    End Sub
    ...
    Private Function IListServer_LocateByExpression(ByVal expression As DevExpress.Data.Filtering.CriteriaOperator, ByVal startIndex As Integer, ByVal searchUp As Boolean) As Integer Implements IListServer.LocateByExpression
        Throw New NotImplementedException()
    End Function
